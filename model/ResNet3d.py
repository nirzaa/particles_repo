...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, sample_size, sample_duration, shortcut_type='B', num_classes=400, last_fc=True):
        ...
        ############################
        ## Costum Summation Layer ##
        self.sum_pool = nn.AvgPool3d((2, 2, 2), stride=1, divisor_override=1)
        ############################

        self.conv1 = nn.Conv3d(1, 64, kernel_size=7, stride=(4, 1, 1),
                               padding=(3, 3, 3), bias=False)       # Some of the models has k_size of 7 others of 3.
        ...
        self.batch_norm = nn.BatchNorm3d(512)
    ...
    def forward(self, x):
        ...
        x = self.sum_pool(x)    # Costum layer
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)
