...
def plotme(xx, yy1, yy2, location):
    ...
    ax1.hist(yy1,xx, fill=False, color='b', linestyle='-', label=r"$N_\mathregular{true}$")
    ax1.hist(yy2,xx, fill=False, color='k', linestyle='--', label=r"$N_\mathregular{recon}$")
    ax2.hist(residue_normalised,xx, color='k')
    ax.set_xlim(-3,3)
    ax2.set_ylim(-1,1)
